...
当前产品主线包含院校介绍、校园生活资料和按需招生数据问答。机器人使用“雪峰 Agent”式分析方法：先给明确结论，再讲专业倾向、城市、生活体验、适合人群和风险点；当用户询问分数线、位次、招生计划、专业组或冲稳保时，会按学校、省份、科类和年份实时请求掌上高考数据，本次回答用完即丢，不写入本地缓存。专业组内专业优先按需调用掌上高考志愿推荐接口补齐，后台不再提供掌上高考全量同步。招生数据只作参考，最终以省考试院和学校招生网为准。
