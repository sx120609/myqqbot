...
describe("admin-auth", () => {
  it("protects management APIs with a signed admin session cookie", async () => {
    const app = Fastify();
    await registerAdminAuth(app, testConfig({ adminPassword: "secret", sessionSecret: "session-secret" }));
  ...
  it("reports a missing admin password instead of leaving APIs open", async () => {
    const app = Fastify();
    await registerAdminAuth(app, testConfig({ adminPassword: "", sessionSecret: "" }));
    app.get("/api/private", async () => ({ ok: true }));
